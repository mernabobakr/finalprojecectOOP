    ...
    private class MouseHandler extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent event) {
        	System.out.println("tozz");
        	 ...
        	 AbstractFactory shapeFactory = FactoryProducer.getFactory(currentShapeFilled);
        	...
        	switch (currentShapeType) //0 for line, 1 for rect, 2 for oval
            {
                ...
                case 1:
                    ...
                    currentShapeObject = shapeFactory.getShape("RECTANGLE",event.getX(), event.getY(),
                            event.getX(), event.getY(), currentShapeColor);
                    System.out.println("nein");
                    break;
                case 2:
                	currentShapeObject = shapeFactory.getShape("OVAL",event.getX(), event.getY(),
                            event.getX(), event.getY(), currentShapeColor);
                    break;
        ...
        public void mouseReleased(MouseEvent event) {
        	...
        	// System.out.println("elawl");
            if (currentShapeObject instanceof FilledRectangle) {
            	System.out.println("elawl");
                ((FilledRectangle) currentShapeObject).setX2(event.getX());
                ((FilledRectangle) currentShapeObject).setY2(event.getY());
                System.out.println("d5lt hena");
        ...
        public void mouseDragged(MouseEvent event) {
            ...
            else if (currentShapeObject instanceof BasicRectangle) {
            	System.out.println("elawl");
                ((BasicRectangle) currentShapeObject).setX2(event.getX());
                ((BasicRectangle) currentShapeObject).setY2(event.getY());
            } 
            else if (currentShapeObject instanceof BasicOval) {
            	System.out.println("elawl");
                ((BasicOval) currentShapeObject).setX2(event.getX());
                ((BasicOval) currentShapeObject).setY2(event.getY());
            }
            else if (currentShapeObject instanceof FilledOval) {
            	System.out.println("elawl");
                ((FilledOval) currentShapeObject).setX2(event.getX());
                ((FilledOval) currentShapeObject).setY2(event.getY());
            }
            else if (currentShapeObject instanceof MyLine) {
                ((MyLine) currentShapeObject).setEndPoint(new Point(event.getX(), event.getY()));
            }
